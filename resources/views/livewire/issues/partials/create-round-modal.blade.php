        ...
        <div class="wp-card wp-card-pad wp-stack-tight wp-surface-muted">
            ...
            <div class="wp-field">
                <x-wp-tooltip :text="__('issues.round_create.stops_help')" wrap>
                    <label class="wp-label" @if($createRoundStopUnitsGrouped->flatten(1)->isNotEmpty()) for="round_create_stop_unit_ids" @endif>{{ __('issues.round_create.stops') }}</label>
                </x-wp-tooltip>
                @include('livewire.issues.partials.round-stop-editor', [
                    ...
                    'pickerId' => 'round_create_stop_unit_ids',
                    'help' => __('issues.round_create.stops_help'),
                ])
            </div>
        </div>
