        ...
        <form x-data
            ...
            @if ($is_recurring)
                <div class="wp-card wp-card-pad wp-stack-tight wp-surface-muted">
                    ...
                    <div class="wp-field">
                        <label class="wp-label" @if($createRoundStopUnitsGrouped->flatten(1)->isNotEmpty()) for="create_round_stop_unit_ids" @endif>{{ __('issues.create.round_stops') }}</label>
                        @include('livewire.issues.partials.round-stop-editor', [
                            'grouped' => $createRoundStopUnitsGrouped,
                            'hiddenCount' => $createRoundStopUnitsHiddenCount,
                            'pickerId' => 'create_round_stop_unit_ids',
                            'help' => __('issues.create.round_stops_help'),
                        ])
                    </div>
    ...
    @else
        <form wire:submit="saveCreateStepTwo" class="wp-card wp-card-pad wp-stack wp-modal-card">
            ...
            <div class="wp-field">
                ...
                <select id="create_internal_team_id" class="wp-select" wire:model.live="internal_team_id">
            ...
            <div class="wp-field">
                <label class="wp-label" for="create_assigned_worker_id">{{ __('issues.create.worker') }}</label>
                <select id="create_assigned_worker_id" class="wp-select" wire:model="assigned_worker_id" @disabled(! $internal_team_id)>
