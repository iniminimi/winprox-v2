@php
    ...
    $focusOnAttention = $hasLoginError && $attentionFocus;
@endphp
<div @class(['wp-auth-login', 'wp-auth-login--attention-focus' => $focusOnAttention])>
    <div class="wp-auth-brand" wire:key="login-brand-{{ $focusOnAttention ? 'attention-'.$attentionTick : ($hasLoginError ? 'attention' : 'logo') }}">
        @if ($hasLoginError)
