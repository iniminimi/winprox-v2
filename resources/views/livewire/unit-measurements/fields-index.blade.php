<div class="wp-stack" data-manual-capture="unit-measurements-fields">
    ...
    <x-wp-modal closeMethod="closeModal" :show="$showModal" :title="$editingFieldId ? __('unit_measurements.fields.edit_title') : __('unit_measurements.fields.create_title')">
        <form class="wp-stack" wire:submit="save">
            <div class="wp-field">
                <label class="wp-label" for="measure-field-name">{{ __('unit_measurements.fields.form.name') }}</label>
                <input id="measure-field-name" type="text" class="wp-input" wire:model="name" maxlength="120">
            ...
            <div class="wp-field">
                ...
                <select id="measure-field-type" class="wp-select" wire:model.live="type" @disabled($editingHasMeasurements)>
            ...
            @if ($type === 'numeric')
                <div class="wp-field">
                    ...
                    <input id="measure-field-uom" type="text" class="wp-input" wire:model="unitOfMeasure" maxlength="32" placeholder="km, °C, L…">
            ...
            @if ($type === 'choice')
                <div class="wp-stack-tight">
                    <p class="wp-section-title">{{ __('unit_measurements.fields.form.options') }}</p>
                    @foreach ($choiceOptions as $index => $option)
                        <div class="wp-cluster" wire:key="choice-opt-{{ $index }}">
                            <input type="text" class="wp-input wp-grow" wire:model="choiceOptions.{{ $index }}" maxlength="120"
                                   @disabled(in_array($option, $lockedChoiceOptions, true) && $option !== '')>
                            @if (! in_array($option, $lockedChoiceOptions, true) || $option === '')
                                <button type="button" class="btn btn--ghost btn--sm" wire:click="removeChoiceOption({{ $index }})">{{ __('common.button.delete') }}</button>
                            @endif
                        </div>
                    @endforeach
                    @error('choiceOptions') <p class="wp-error">{{ $message }}</p> @enderror
                    <button type="button" class="btn btn--ghost btn--sm" wire:click="addChoiceOption">{{ __('unit_measurements.fields.form.add_option') }}</button>
                </div>
            @endif
