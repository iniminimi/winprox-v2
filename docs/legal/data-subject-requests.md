...
- **Operationele lijstrapporten** (niet AVG-SAR): op zoekschermen (meldingen, taken, Time, reserveringen, unit checks, unitmetingen, ESG) kunnen medewerkers met toegang tot die lijsten een gefilterde **CSV** of **afdruk** downloaden via **Download rapport** (afdruk volgt waar mogelijk de backoffice-lijstlayout; Time = urenstaat). Dit is geen kopie van alle persoonsgegevens van een betrokkene — dat blijft de account-export hierboven.
