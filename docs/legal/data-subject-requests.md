...
- **Operationele lijstrapporten** (niet AVG-SAR): op zoekschermen (meldingen, taken, Time, reserveringen, unit checks, unitmetingen, ESG) kunnen medewerkers met toegang tot die lijsten een gefilterde **CSV** of **afdruk** downloaden via **Download rapport** (afdruk volgt waar mogelijk de backoffice-lijstlayout; Time = urenstaat, inclusief eventuele teampauze-minuten). Dit is geen kopie van alle persoonsgegevens van een betrokkene — dat blijft de account-export hierboven.
- **Clock Point Mijn uren**: de uitvoerder ziet alleen de eigen diensten van de gekozen maand; geen export van collega’s. De getoonde pauze is de urenstaat (kan een teamminimum bevatten) en is geen extra RSZ IN/OUT.
