...
**Werkmenu instellingen** (Organisatie → Instellingen, admin): per tenant Kalender, Reserveringen,
...
- Modal: **kies je type organisatie** (werken op locatie, hotel, ziekenhuis, industrie, gemeentebestuur, vastgoedbeheer, fleet management). Bij **hotel, werken op locatie en vastgoedbeheer** ook de **grootte** (1–5 / 6–20 / 21–100 werknemers). Uitleg wat er in de DB komt, inclusief **werkmenu** (aan/uit per sidebar-item). Kleine ploegen: één team, extra-werkmenu uit. Knoppen **Aanmaken** / **Annuleren**. Dit is **geen sectorsysteem** — alleen een starttemplate.
- Na aanmaken: kaart **Starttemplate aangemaakt** met het resultaat (teams, categorieën, locatie, units **en werkmenu**), volgende stap in het rood, pulserende knop **Ga naar units via Locaties** en knop **Verwijder starttemplate**. De kaart verdwijnt automatisch na 7 dagen (of eerder via **Sluiten**). Werkmenu later wijzigen via **Organisatie → Instellingen → Werkmenu instellingen**.
- Stelt tegelijk passende **werkmenu**-defaults in (sidebar): vastgoedbeheer en werken op locatie zonder Reserveringen/Unitmetingen; 1–5 werknemers (hotel, werken op locatie, vastgoed) met extra-werkmenu uit; gemeentebestuur, ziekenhuis, industrie en fleet management met Kalender, Reserveringen, Inspectierondes en Unitmetingen aan (portaal-vinkjes op categorieën/units blijven apart — hotel activeert unitmetingen niet automatisch op units).
...
- **Beheer** (`/unit-measurements`): pagina Unitmetingen — uitvouwbare Meetvelden + zoekfilters + resultaten.
  ...
  (km, temperatuur, urenstand, status, brandstof, voorraad, bezetting) vullen het formulier voor via dropdown.
- **Download rapport** (`x-wp-list-export`): CSV + afdrukken (kaartlayout) van de gefilterde
...
- **Plek (groep/unit):** optioneel in dezelfde cel: `D1/G1` of `07:00-12:00/G1` (spatie mag;
  ...
  aan die locatie. Vlinders/invallers (`clocks_all_locations` of zonder locatie) blijven
  overal inklokken, maar verschijnen niet op het locatie-uurrooster (wél in de platte lijst
  zonder locatiefilter). Mijn rooster toont snapshot-`unit_name`.
...
- **Eén open WorkShift** per worker; op een andere Clock Point toont het portaal waar je bent ingeklokt + **Verplaats naar hier** (zelfde dienst: `presence_clock_point_id` + hop in `location_hops`, geen nieuwe shift).
- **Thuisgroep (uurrooster):** optioneel `workers.default_unit_id` → unit met `roster_code` (zie §5g.5). Cel-plek `D1/G1` blijft apart.
...
- Volgorde: **tenant-inzicht** (DB-tellingen) → **kennisbank** → **pagina-hulp-patronen**
  ...
  **Geen automatische helpdesk-mail** — na elk antwoord knop **Doorsturen naar helpdesk**
