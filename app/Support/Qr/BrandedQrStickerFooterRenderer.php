...
/** Sticker number centered below the QR on Avery 62×89-R branded artwork. */
final class BrandedQrStickerFooterRenderer
{
    /**
    ...
    public static function drawOnGd($canvas, string $text): void
    {
        ...
        [$r, $g, $b] = Avery62x89StickerArtworkLayout::FOOTER_COLOR_RGB;
        ...
        if (! is_array($bbox)) {
            throw new \RuntimeException('Unable to measure branded sticker footer text.');
        }

        $ascent = abs($bbox[7]);
        $y = Avery62x89StickerArtworkLayout::FOOTER_TOP_PX + $ascent;
    ...
    public static function drawOnImagick(Imagick $canvas, string $text): void
    {
        ...
        $y = (float) Avery62x89StickerArtworkLayout::FOOTER_TOP_PX + $ascent;
    ...
    private static function fitFontSize(string $text, string $fontPath): int
    {
        ...
        for ($fontSize = Avery62x89StickerArtworkLayout::FOOTER_MAX_FONT_SIZE_PX;
            $fontSize >= Avery62x89StickerArtworkLayout::FOOTER_MIN_FONT_SIZE_PX;
            $fontSize--) {
        ...
        return Avery62x89StickerArtworkLayout::FOOTER_MIN_FONT_SIZE_PX;
    }
    ...
    private static function textWidth(string $fontPath, int $fontSize, string $text): int
    {
        ...
        if (! is_array($bbox)) {
            throw new \RuntimeException('Unable to measure branded sticker footer text width.');
        }
